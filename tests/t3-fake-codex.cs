...
public class T3FakeCodex {
    ...
    public static int Main(string[] args) {
        ...
        while((line=Console.ReadLine()) != null) {
            ...
            if(method=="account/rateLimits/read") result=new {rateLimits=new {limitId="codex",primary=new {usedPercent=File.Exists(Path.Combine(home,"exhausted"))?100:10,windowDurationMins=10080,resetsAt=DateTimeOffset.UtcNow.ToUnixTimeSeconds()+3600},secondary=(object)null,spendControlReached=false,rateLimitReachedType=(object)null}};
            ...
            if(method=="turn/start") {
                Send(new {method="turn/started",@params=new {threadId="thread-fixture",turn=new {id="turn-fixture"}}});
                Send(new {id=id,result=new {turn=new {id="turn-fixture"},account=account}});
                Send(new {method="turn/completed",@params=new {threadId="thread-fixture",turn=new {id="turn-fixture",status="completed"}}});
                continue;
            }
