...
The main bar never substitutes weekly inventory for current availability. Weekly remaining is small supporting text. When exact window conversions are missing, known plan session multipliers weight a quota-headroom estimate using each account's tightest remaining limit. This is an approximation of headroom, not a measured amount of compute; it does not silently populate the conversion catalogue. Missing readings or multi-account plan weights remain unknown. A paused/held/manual Claude configuration can leave allowance on other accounts outside the solid routed amount. The Codex summary always uses main Codex, excluding Spark; existing sessions retain their native account.
