...
# -PlanOnly writes the launcher and reports the task it would register without touching
# the scheduler, so a test can execute the launcher that keeps installations current.
...
if($PlanOnly){@{name=$name;description=$description;launcher=$launcher}|ConvertTo-Json;exit 0}
